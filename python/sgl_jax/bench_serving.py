...
def _build_profile_urls(
    profile_prefill_url: list[str] | None,
    profile_decode_url: list[str] | None,
) -> list[tuple[str, str]]:
...
def sample_mmmu_requests(
    ...
    try:
        ...
        raise ValueError(f"Failed to load MMMU dataset: {e}") from e
...
def create_mm_data_row(text_prompt, images: list, images_base64, output_len, processor, backend):
    try:
        if type(processor).__name__ == "Phi4MMProcessor":
            ...
            content_items = [
                {"type": "image", "image": {"url": image_base64}} for image_base64 in images_base64
            ]
...
def sample_generated_shared_prefix_requests(
    ...
    for group_idx in tqdm(range(num_groups), desc="Generating system prompt"):
        ...
        routing_key = (
            f"{run_random_str}_{run_start_timestamp}_{group_idx}" if send_routing_key else None
        )
        ...
        if outputs[i].success:
            ...
            if use_retokenized_itl:
                for k, itl in enumerate(outputs[i].itl):
                    num_tokens = len(
                        tokenizer.encode(outputs[i].text_chunks[k], add_special_tokens=False)
                    )
...
async def benchmark(
    ...
    disable_tqdm: bool,
    lora_names: list[str],
    lora_request_distribution: str | None,
    lora_zipf_alpha: float | None,
    extra_request_body: dict[str, Any],
    profile: bool,
    ...
    profile_prefill_url: list[str] | None = None,
    profile_decode_url: list[str] | None = None,
):
    ...
    print("{:<40} {:<10}".format("Peak concurrent requests:", metrics.max_concurrent_requests))
...
def run_benchmark(args_: argparse.Namespace):
    ...
    if args.backend in ["sglang", "sgl-jax", "sglang-native"]:
        api_url = (
    ...
    if args.dataset_name in ["image", "mmmu"]:
        ...
        assert not args.tokenize_prompt, "`--tokenize-prompt` not compatible with image dataset"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--tag", type=str, default=None, help="The tag to be dumped to output.")
    parser.add_argument(
        "--header",
        type=str,
        nargs="+",
